 ...
 * on a phone, not at a desk. Minimum fields only." Big touch targets,
 * one field group, one action — nothing else on the screen competing for
 * attention. Confirms in place and resets for the next customer rather
 * than navigating away, since a tradesperson doing this back-to-back
 * after several jobs is the expected pattern, not a one-off.
 */
export function QuickAddForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await quickAddContact(name.trim(), phone.trim(), email.trim());
    setSubmitting(false);
      ...
      <div className="flex flex-col items-start gap-4 rounded-lg border border-success/40 bg-success/10 p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            You can close this now, or add the next customer.
          </p>
        </div>
