...
export function TemplateEditor({ initialTemplates }: { initialTemplates: TemplateData[] }) {
  ...
  return (
    <div className="flex w-full max-w-2xl flex-col gap-6">
      <div aria-label="Template step" className="flex gap-2 border-b border-border">
        {STEPS.map((step) => (
          <button
            key={step}
            type="button"
      ...
      <div aria-live="polite" className="min-h-10">
        {checking ? (
          ...
          <div className="rounded-lg border border-warning/40 bg-warning/10 p-3 text-sm">
            <p className="font-medium text-foreground">This message needs changes:</p>
            ...
            {result.suggested_rewrite && (
              <div className="mt-3 flex flex-col gap-2">
                ...
                <p className="rounded border border-border bg-background p-2 italic text-foreground">
                  {result.suggested_rewrite}
                </p>
                <div>
                  <Button
